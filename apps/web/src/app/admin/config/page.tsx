...
export default async function ConfigPage() {
  const [questionCount, trackCount, templateCount] = await Promise.all([
    prisma.intakeQuestion.count(),
    prisma.programTrack.count(),
    prisma.emailTemplate.count(),
  ]);

  const checklistTemplateCount = await prisma.programTrack.count({
    where: {
      NOT: { checklistTemplate: "[]" },
    },
  });
  ...
  const configSections = [
    ...
    },
    {
      ...
      description: "Define tax workflow tracks and associated checklist templates.",
      icon: "🗂️",
      href: "/admin/config/intake",
      count: `${trackCount} tracks`,
    },
    {
      title: "Checklist Templates",
      description: "Configure document requirements for each workflow track.",
      icon: "✅",
      href: "/admin/config/intake",
      count: `${checklistTemplateCount} templates`,
    },
    {
